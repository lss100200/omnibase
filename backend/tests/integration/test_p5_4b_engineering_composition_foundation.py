...
def _seed(db_engine) -> None:  # type: ignore[no-untyped-def]
    ...
    with db_engine.begin() as connection:
        ...
        connection.execute(
            text(
                "INSERT INTO omnibase_meta.resource_registry (id,tenant_id,kind,owner_type,owner_id,display_name,state,version,policy_class) VALUES (:id,:tenant,'workspace','system',NULL,'P5.4B workspace','active',1,'workspace_private') ON CONFLICT (id) DO NOTHING"
            ),
        ...
        connection.execute(
            ...
            ),
            {
                ...
                "actor": ACTOR,
                "resource": RESOURCE,
                "constraints": '{"max_result_bytes":1048576,"timeout_ms":3000}',
            },
        )
        connection.execute(
            text(
...
def test_engineering_composition_seeds_and_executes_gateway_backed_search(db_engine) -> None:  # type: ignore[no-untyped-def]
    ...
    core = CoreVerifiedCapability(
        ...
        constraints={"max_result_bytes": 1_048_576, "timeout_ms": 3000},
    )
    capability = VerifiedCapability(
        ...
        constraints=CapabilityConstraints(max_bytes=1_048_576, max_timeout_ms=3000),
