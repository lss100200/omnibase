...
const ELECTRON_ZIP_NAME = "electron-v43.4.0-linux-x64.zip";
...
function parseArguments(argv) {
  if (argv.length !== EXPECTED_ARGUMENTS.size * 2) {
    fail("desktop_linux_packager_arguments_invalid");
  }
  ...
  const zipSha256 = values.get("--electron-zip-sha256");
  if (!SHA256.test(zipSha256))
    fail("desktop_linux_packager_electron_sha_invalid");
  return Object.freeze({
    appDir: values.get("--app-dir"),
    electronZipDir: values.get("--electron-zip-dir"),
    electronZipSha256: zipSha256,
...
async function requireExecutableFile(file, code) {
  await requireOrdinaryFile(file, code);
  let metadata;
  try {
    metadata = await lstat(file);
  } catch {
  ...
  if (process.platform === "linux" && (metadata.mode & 0o111) === 0) fail(code);
}
...
export async function packageLinux(argv, dependencies = {}) {
  const options = parseArguments(argv);
  if (!VERSION.test(options.version))
    fail("desktop_linux_packager_version_invalid");
  const appDir = await requireOrdinaryDirectory(
    options.appDir,
    "desktop_linux_packager_app_dir_invalid",
  );
  const runtimeDir = await requireOrdinaryDirectory(
    options.runtimeDir,
    "desktop_linux_packager_runtime_dir_invalid",
  );
  const electronZipDir = await requireOrdinaryDirectory(
    options.electronZipDir,
    "desktop_linux_packager_electron_dir_invalid",
  );
  const outputDir = await requireOrdinaryDirectory(
    options.outputDir,
    "desktop_linux_packager_output_dir_invalid",
  );
  if (path.basename(runtimeDir) !== "runtime") {
    fail("desktop_linux_packager_runtime_dir_name_invalid");
  }
  await requireOrdinaryFile(
    path.join(appDir, "dist", "main.js"),
    "desktop_linux_packager_main_entry_invalid",
  );
  await requireOrdinaryFile(
    path.join(runtimeDir, "runtime-manifest.json"),
    "desktop_linux_packager_runtime_manifest_invalid",
  );
  const electronZip = path.join(electronZipDir, ELECTRON_ZIP_NAME);
  await requireOrdinaryFile(
    electronZip,
    "desktop_linux_packager_electron_zip_invalid",
  );
  const expectedElectronZipSha256 =
    dependencies.expectedElectronZipSha256 ?? options.electronZipSha256;
  if ((await digestFile(electronZip)) !== expectedElectronZipSha256) {
    fail("desktop_linux_packager_electron_zip_digest_mismatch");
  }
  const runtimeSnapshot = await snapshotTree(
    runtimeDir,
    "desktop_linux_packager_runtime_tree_invalid",
  );
  const target = path.join(outputDir, "OmniBase-linux-x64");
  try {
    await lstat(target);
    fail("desktop_linux_packager_target_exists");
  } catch (error) {
    if (error?.message === "desktop_linux_packager_target_exists") throw error;
    if (error?.code !== "ENOENT")
      fail("desktop_linux_packager_target_identity_invalid");
  }
  ...
  if (
    ...
    fail("desktop_linux_packager_output_invalid");
  }
  await requireOrdinaryDirectory(
    target,
    "desktop_linux_packager_output_invalid",
  );
  await requireExecutableFile(
    path.join(target, "OmniBase"),
    "desktop_linux_packager_output_entry_invalid",
  );
  await requireOrdinaryFile(
    path.join(target, "resources", "app.asar"),
    "desktop_linux_packager_output_asar_invalid",
  );
  const packagedRuntime = path.join(target, "resources", "runtime");
  await requireOrdinaryDirectory(
    packagedRuntime,
    "desktop_linux_packager_output_runtime_invalid",
  );
  if (
    (await snapshotTree(
      packagedRuntime,
    ...
    fail("desktop_linux_packager_output_runtime_mismatch");
  }
  return target;
}
...
async function main() {
  try {
    const target = await packageLinux(process.argv.slice(2));
    process.stdout.write(`${JSON.stringify({ output_dir: target })}\n`);
  } catch (error) {
    const code =
      error instanceof Error &&
      /^desktop_linux_packager_[a-z_]+$/u.test(error.message)
        ? error.message
        : "desktop_linux_packager_failed";
