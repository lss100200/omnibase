"""Build the minimal desktop-local backend as a closed Linux onedir bundle."""
...
EXPECTED_EXECUTABLE = "OmniBase.Desktop.Backend"
MAX_FILES = 512
MAX_TOTAL_BYTES = 512 * 1024 * 1024


class LinuxBackendBuildError(ValueError):
    """A stable, path-redacted Linux backend build failure."""


def _ordinary_directory(path: Path, *, must_exist: bool, code: str) -> Path:
    absolute = path.absolute()
    if not absolute.is_absolute():
        raise LinuxBackendBuildError(code)
    if not must_exist:
    ...
    try:
        ...
        raise LinuxBackendBuildError(code) from None
    if (
...
def _require_outside_repository(path: Path, repository: Path) -> None:
    try:
        ...
        return
    raise LinuxBackendBuildError("linux_backend_output_inside_repository")
...
def _verify_build_runtime() -> None:
    if (
        ...
        raise LinuxBackendBuildError("linux_backend_build_runtime_unsupported")
    for package, expected in EXPECTED_PACKAGES.items():
        try:
            actual = importlib.metadata.version(package)
        except importlib.metadata.PackageNotFoundError:
            raise LinuxBackendBuildError(
                "linux_backend_build_dependency_missing"
            ) from None
        if actual != expected:
            raise LinuxBackendBuildError(
                "linux_backend_build_dependency_version_mismatch"
            )
...
def _validate_output(publish_dir: Path) -> tuple[int, int]:
    try:
        ...
        raise LinuxBackendBuildError("linux_backend_publish_missing") from None
    if not stat.S_ISDIR(root_metadata.st_mode) or stat.S_ISLNK(root_metadata.st_mode):
        raise LinuxBackendBuildError("linux_backend_publish_identity_invalid")
    ...
    for directory, directory_names, file_names in os.walk(publish_dir):
        ...
        for name in (*directory_names, *file_names):
            ...
            if stat.S_ISLNK(metadata.st_mode):
                raise LinuxBackendBuildError("linux_backend_publish_link_forbidden")
        for name in file_names:
            ...
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_size <= 0:
                raise LinuxBackendBuildError("linux_backend_publish_file_invalid")
            count += 1
            total += metadata.st_size
            if count > MAX_FILES or total > MAX_TOTAL_BYTES:
                raise LinuxBackendBuildError("linux_backend_publish_budget_exceeded")
            if target == publish_dir / EXPECTED_EXECUTABLE:
                executable_seen = bool(metadata.st_mode & 0o111)
    if not executable_seen:
        raise LinuxBackendBuildError("linux_backend_publish_entrypoint_missing")
    return count, total
...
def build_backend(repo_root: Path, dist_dir: Path, work_dir: Path) -> dict[str, object]:
    ...
    repository = _ordinary_directory(
        repo_root, must_exist=True, code="linux_backend_repository_invalid"
    )
    distribution = _ordinary_directory(
        dist_dir, must_exist=False, code="linux_backend_dist_path_invalid"
    )
    work = _ordinary_directory(
        work_dir, must_exist=False, code="linux_backend_work_path_invalid"
    )
    ...
    if os.path.normcase(str(distribution)) == os.path.normcase(str(work)):
        raise LinuxBackendBuildError("linux_backend_build_paths_must_differ")
    _ordinary_directory(
        distribution.parent,
        must_exist=True,
        code="linux_backend_dist_parent_invalid",
    )
    _ordinary_directory(
        work.parent, must_exist=True, code="linux_backend_work_parent_invalid"
    )
    ...
    if not spec.is_file() or spec.is_symlink():
        raise LinuxBackendBuildError("linux_backend_spec_invalid")
    command = [
        sys.executable,
        "-m",
        "PyInstaller",
    ...
    try:
        ...
        raise LinuxBackendBuildError("linux_backend_packager_failed") from None
...
def main() -> int:
    ...
    except LinuxBackendBuildError as exc:
        import json
