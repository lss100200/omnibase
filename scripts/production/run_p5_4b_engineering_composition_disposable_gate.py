"""Run the current-baseline P5.4B Capability Gateway disposable Gate.
...
``omnibase_test_p54b_*`` database with mandatory cleanup.
"""
...
GATE_NAME = "P5.4B engineering composition disposable Gate"
TEMP_ROOT = (REPO_ROOT / ".tmp" / "p5-4b-engineering-composition-gate").resolve()
...
INTEGRATION_TESTS = ("tests/integration/test_p5_4b_engineering_composition_foundation.py",)
...
def _run_gate_steps(project: str, database_url: str) -> None:
    commands = (
        ...
        (
            "P5.4B engineering composition integration",
...
def _record(
    *,
    ...
    report: dict[str, object] = {
        ...
        "typed_executor_composition_verified": passed,
        "seed_and_execute_verified": passed,
    ...
    _write(
        ...
        "\n".join(
            (
                "# P5.4B engineering composition disposable Gate",
                "",
                f"- Passed: `{passed}`",
...
def _verify(path: Path) -> None:
    ...
    expected = {
        ...
        "typed_executor_composition_verified": True,
        "seed_and_execute_verified": True,
        "production_runtime_activated": False,
        "feature_gates_enabled": False,
        "migration_0013_created": False,
        "root_env_accessed": False,
    ...
    for key, value in expected.items():
        if report.get(key) != value:
            raise RuntimeError(f"P5.4B evidence mismatch: {key}")
    if report.get("source_manifest_sha256") != _manifest_digest(_manifest()):
        raise RuntimeError("P5.4B source manifest drifted")
...
def main() -> int:
    ...
    if args.validate_only:
        print("P5.4B current-baseline disposable Gate static validation passed")
        return 0
    if args.verify_evidence is not None:
        _verify(args.verify_evidence)
        print("P5.4B current-baseline evidence seal passed")
        return 0
    if _dirty_paths():
        raise RuntimeError("P5.4B Gate requires a clean checkout")

    stamp = datetime.now(UTC).strftime("%Y%m%d%H%M%S")
    project = f"omnibase-test-p54b-{stamp}"
    database_name = f"omnibase_test_p54b_{stamp.lower()}"
    role_name = f"omnibase_test_p54b_{stamp[-8:]}"
    run_dir = (TEMP_ROOT / stamp).resolve()
    if TEMP_ROOT not in run_dir.parents:
        raise RuntimeError("P5.4B temporary path escaped .tmp")
    ...
    try:
        ...
        if up.returncode != 0:
            raise RuntimeError(
                "P5.4B postgres-test startup failed:\n" + up.stdout[-3000:]
            )
        ...
        print("P5.4B current-baseline disposable Gate passed")
        return 0
        ...
        print(f"P5.4B current-baseline disposable Gate failed: {exc}", file=sys.stderr)
        return 1
    finally:
        if not cleanup_complete:
            with suppress(Exception):
