#!/usr/bin/env python3
"""Validate the compile-only P5.5A Memory / ContextCapsule contract.

The command parses privacy, provenance, lifecycle and budget examples only. It
never reads the root .env, connects to a database/provider, creates a migration,
persists memory, exposes an API, compiles a live capsule or injects memory into
an Agent.
...
_contract = importlib.import_module("omnibase.production.phase5_memory_contract")
AdmissionState = _contract.AdmissionState
ConfigurationError = _contract.ConfigurationError
MemoryContractGate = _contract.MemoryContractGate
load_memory_contract_config = _contract.load_memory_contract_config

DEFAULT_CONFIG = (
    DEFAULT_REPO_ROOT
    / "deployment"
    / "production"
    / "phase5-memory-contract.example.json"
)
    ...
    for part in relative.parts:
        ...
        if stat.S_ISLNK(metadata.st_mode) or _is_reparse(metadata):
            raise ConfigurationError(
                "configuration path contains a link or reparse point"
            )
    metadata = os.lstat(candidate)
...
def _reject_link_components(path: Path) -> None:
    ...
    for part in absolute.parts[1:]:
        ...
        if stat.S_ISLNK(metadata.st_mode) or _is_reparse(metadata):
            raise ConfigurationError(
                "report output path contains a link or reparse point"
            )
...
def main() -> int:
    ...
    try:
        config = load_memory_contract_config(
            _safe_config_path(arguments.config, repo_root)
        )
