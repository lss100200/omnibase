...
def _repo_fixture(repo: Path, *, head: str = "0016") -> None:
...
def _prepare_backup(tmp_path: Path, *, head: str = "0016") -> tuple[Path, Path]:
    repo = tmp_path / "repo"
    repo.mkdir()
...
def _stage(tmp_path: Path, *, head: str = "0016") -> tuple[Path, Path]:
...
def test_complete_backup_and_restore_plan_are_offline_and_bound(tmp_path: Path) -> None:
    ...
    assert verified["source_migration_head"] == "0016"
    ...
    assert restore["source_migration_head"] == "0016"
    assert restore["target_migration_head"] == "0016"
    ...
    assert manifest["source_migration_head"] == "0016"
    ...
    assert len(manifest["migration_0016_schema_sha256"]) == 64
...
def test_0014_backup_requires_canonical_0015_memory_bootstrap_upgrade_entry(
    ...
    assert restore["migration_0016_schema_sha256"] is None


def test_0015_backup_requires_the_only_canonical_0016_model_override_upgrade_entry(
    tmp_path: Path,
) -> None:
    repo, target = _stage(tmp_path, head="0015")
    _write_migration(repo, "0016", "0015")
    inventory = _inventory(tmp_path / "databases.json")
    arguments = {
        "repo_root": str(repo),
        "backup_target": str(target),
        "target_database": "omnibase_restore_model_override_upgrade",
        "database_inventory": str(inventory),
        "minio_restore_root": str(tmp_path / "restored-model-override-minio"),
    }
    with pytest.raises(backup.BackupError, match="canonical compatibility entry"):
        backup.plan_restore(Namespace(**arguments))
    with pytest.raises(backup.BackupError, match="canonical compatibility entry"):
    ...
    restore = backup.plan_restore(
        Namespace(
            **arguments,
            compatibility_entry="p6-personal-model-overrides-0015-to-0016",
        )
    )
    assert restore["migration_mode"] == "canonical_compatibility_upgrade"
    ...
    assert restore["target_migration_head"] == "0016"
    assert restore["compatibility_entry"]["entry_id"] == (
        "p6-personal-model-overrides-0015-to-0016"
    )
    assert len(restore["migration_0016_schema_sha256"]) == 64
    assert (
...
@pytest.mark.parametrize("revision", ["0007", "0013", "0014", "0015", "0016"])
...
def test_seal_rejects_repository_advancing_after_plan(tmp_path: Path) -> None:
    ...
    _write_migration(repo, "0017", "0016")
    with pytest.raises(backup.BackupError, match="migration 0017 or higher"):
        backup.seal_assets(
            Namespace(
                repo_root=str(repo),
                backup_target=str(target),
...
def test_0015_to_0016_restore_rejects_any_noncanonical_compatibility_entry(
    tmp_path: Path,
) -> None:
    repo, target = _stage(tmp_path, head="0015")
    _write_migration(repo, "0016", "0015")
...
def test_repository_head_0017_or_higher_is_not_admitted(tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    _repo_fixture(repo)
    _write_migration(repo, "0017", "0016")
    with pytest.raises(backup.BackupError, match="migration head 0016 only"):
        backup.plan_backup(
            Namespace(
                repo_root=str(repo),
                backup_target=str(tmp_path / "cold" / "backup-0017"),
                source_database="omnibase",
            )
        )
