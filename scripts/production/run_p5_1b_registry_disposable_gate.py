"""Run and clean the guarded P5.1B Agent Registry persistence disposable Gate.

The Gate provisions a fresh disposable PostgreSQL database (isolated Compose
project, ``omnibase_test_*`` names, sentinel, restricted non-owner role),
migrates it to head, runs the guarded P5.1B integration suite, records
fail-closed evidence, and tears the disposable database down.
...
_SOURCE_MANIFEST_PATHS = (
    ...
    "backend/tests/test_p5_1b_agent_registry.py",
...
def _verify_recorded_evidence(report: dict[str, object]) -> None:
    if report.get("schema_version") != 1:
        raise RuntimeError("P5.1B evidence schema version mismatch")
    if report.get("gate") != "P5.1B Agent Registry persistence disposable Gate":
        raise RuntimeError("P5.1B evidence gate identity mismatch")
    ...
    if not isinstance(recorded, str) or recorded != _manifest_sha256(manifest):
        raise RuntimeError("P5.1B source manifest drifted since the recorded Gate")
...
def main() -> int:
    ...
    if arguments.verify_evidence is not None:
        ...
        try:
            ...
            raise RuntimeError("P5.1B evidence path escaped the repository") from exc
        report = json.loads(evidence_path.read_text(encoding="utf-8"))
        if not isinstance(report, dict):
            raise RuntimeError("P5.1B evidence root is not an object")
        _verify_recorded_evidence(report)
        print("P5.1B recorded evidence source seal passed")
        return 0

    _validate_static_contract()
    if arguments.validate_only:
        print("P5.1B Gate static contract valid")
        return 0

    stamp = datetime.now(UTC).strftime("%Y%m%d%H%M%S")
    project = f"omnibase-p51b-{stamp}"
    database_name = f"omnibase_test_p51b_{stamp.lower()}"
    role_name = f"omnibase_test_p51b_{stamp[-8:]}"
