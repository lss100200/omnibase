...
deletes its own evidence.  It never activates production Runtime, never reads
  ...
  Phase 5 production Feature Gate. Migration ``0016`` is the current repository
...
def _discover_migration_head() -> str:
    ...
    if 16 not in numeric or any(value >= 17 for value in numeric):
        raise RuntimeError("P5.4C migration filename boundary is not exactly 0016")
    return "0016"
...
def _validate_config() -> None:
    ...
    if config.get("migration_baseline") != EXPECTED_MIGRATION_HEAD:
        raise RuntimeError("P5.4C Gate requires migration baseline 0016")
