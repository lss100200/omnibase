"""Run the guarded engineering-only P5.2C Agent Alpha disposable Gate.

The Gate uses an isolated ``omnibase_test_p52c_*`` PostgreSQL database,
executes the destructive preflight before Alembic, verifies migration head
``0011`` and the P5.2C integration suite (a real HTTP/SSE Alpha invocation
through the engineering composition seam with the deterministic fake Model
Gateway), proves Docker cleanup, and seals the tested source bytes.  It never
reads the root ``.env``, never touches a business database and never contacts
an external network or model provider.  Passing this Gate activates only the
engineering Alpha: ``production_runtime_activated`` stays false and all three
Phase 5 Feature Gates stay false.
"""
...
TEMP_ROOT = (REPO_ROOT / ".tmp" / "p5-2c-agent-alpha-gate").resolve()
EVIDENCE_JSON = REPO_ROOT / "docs/evidence/p5-2/phase5-agent-alpha-engineering-gate.json"
EVIDENCE_MD = REPO_ROOT / "docs/evidence/p5-2/phase5-agent-alpha-engineering-gate.md"
GATE_NAME = "P5.2C Agent Alpha engineering disposable Gate"
INTEGRATION_TEST = "tests/integration/test_p5_2c_agent_alpha_foundation.py"
_SOURCE_PATHS = (
    "AGENTS.md",
    "backend/alembic.ini",
    ...
    "backend/src/omnibase/model_gateway/__init__.py",
    "backend/src/omnibase/model_gateway/contracts.py",
    "backend/src/omnibase/model_gateway/providers.py",
    "backend/src/omnibase/model_gateway/service.py",
    ...
    "backend/src/omnibase/workspace_data/tenant_models.py",
    ...
    "backend/src/omnibase/rag/llm.py",
    "backend/src/omnibase/rag/retriever.py",
    ...
    "backend/tests/integration/test_p5_2c_agent_alpha_foundation.py",
    ...
    "frontend/lib/api.ts",
    "scripts/production/run_p5_2c_agent_alpha_disposable_gate.py",
)
...
def _manifest() -> dict[str, object]:
    ...
    for relative in _SOURCE_PATHS:
        ...
        if path.is_symlink() or not path.is_file():
            raise RuntimeError(f"P5.2C source path is not a regular file: {relative}")
...
def _dirty_paths() -> tuple[str, ...]:
    ...
    if result.returncode != 0:
        raise RuntimeError("P5.2C Gate could not inspect Git status")
    allowed = {
...
def _resource_counts(project: str) -> dict[str, int]:
    ...
    for kind, command in commands.items():
        result = _run(command)
        if result.returncode != 0:
            raise RuntimeError(f"P5.2C Gate could not inspect {kind}")
...
def _cleanup(project: str, env: dict[str, str]) -> dict[str, int]:
    ...
    if down.returncode != 0 or any(counts.values()):
        raise RuntimeError(f"P5.2C cleanup failed: {counts}\n{down.stdout[-1000:]}")
    return counts
...
def _publish(source: Path, destination: Path) -> None:
    if source.is_symlink() or not source.is_file():
        raise RuntimeError("P5.2C evidence source is not a regular file")
    if destination.parent.is_symlink():
        raise RuntimeError("P5.2C evidence parent is a symlink")
...
def _record(
    ...
    *,
    ...
    report: dict[str, object] = {
        ...
        "engineering_alpha_ready": passed,
        ...
        "model_identity_verified": passed,
        "tool_free_verified": passed,
        "tenant_scope_verified": passed,
        "cancellation_scope_verified": passed,
    ...
    _write(
        ...
        "\n".join(
            (
                "# P5.2C Agent Alpha engineering disposable Gate",
                "",
                f"- Passed: {passed}",
                f"- Engineering Alpha ready: {passed}",
                ...
                "- Tool-free verified: true",
                "- Tenant scope verified: true",
                "- Cancellation scope verified: true",
                "- Unknown no-replay verified: true",
...
def _verify_evidence(path: Path) -> None:
    ...
    try:
        ...
        raise RuntimeError("P5.2C evidence escaped the repository") from exc
    ...
    for key, value in expected.items():
        if report.get(key) != value:
            raise RuntimeError(f"P5.2C evidence field mismatch: {key}")
    current = _manifest_digest(_manifest())
    if report.get("source_manifest_sha256") != current:
        raise RuntimeError("P5.2C source manifest drifted")
    if _dirty_paths():
        raise RuntimeError("P5.2C evidence verification requires a source-clean checkout")
...
def main() -> int:
    ...
    if not ENV_FILE.is_file() or not COMPOSE_FILE.is_file():
        raise RuntimeError("P5.2C disposable Gate prerequisites are missing")
    manifest = _manifest()
    if args.validate_only:
        print("P5.2C disposable Gate static contract valid")
        return 0
    if args.verify_evidence is not None:
        _verify_evidence(args.verify_evidence)
        print("P5.2C recorded evidence source seal passed")
        return 0
    dirty = _dirty_paths()
    if dirty:
        raise RuntimeError("P5.2C Gate requires a clean checkout: " + ", ".join(dirty))

    stamp = datetime.now(UTC).strftime("%Y%m%d%H%M%S")
    project = f"omnibase-p52c-{stamp}"
    database_name = f"omnibase_test_p52c_{stamp.lower()}"
    role_name = f"omnibase_test_p52c_{stamp[-8:]}"
    run_dir = (TEMP_ROOT / stamp).resolve()
    if TEMP_ROOT not in run_dir.parents:
        raise RuntimeError("P5.2C temporary path escaped .tmp")
    ...
    database_url = (
        f"postgresql+psycopg://{role_name}:{role_password}" f"@localhost:55433/{database_name}"
    )
    env = dict(os.environ)
    env.update(
        {
    ...
    try:
        ...
        if up.returncode != 0:
            raise RuntimeError("P5.2C PostgreSQL startup failed:\n" + up.stdout[-3000:])
        ...
        print("P5.2C disposable Gate passed")
        return 0
        ...
        print(f"P5.2C disposable Gate failed: {exc}", file=sys.stderr)
        return 1
    finally:
        if not cleanup_complete:
