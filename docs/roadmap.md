...
> **当前 P7.1 入口**：Owner 已批准 Wave 1 的只读本地文件合同，权威产品法为 `docs/architecture/p7-1-local-development-loop.md`。本轮只允许 Owner 选择一个内存态根目录、按 Workspace/generation 浏览有界逻辑文件树并只读打开有界 UTF-8 文本；源码实现、攻击矩阵与全量门禁已有工程证据并等待 PR 审核，真实 Electron 旅程和新安装包仍待证明。保存/写入、Agent 文件工具、Terminal、Git、搜索、重命名/删除、Next/backend 文件路由和数据库迁移继续关闭。
...
| P7.1 Wave 1：只读本地文件 | 工程证据就绪；等待 PR 审核与真实 Electron 旅程 | Owner 原生目录选择器建立一个仅内存、Workspace/generation 绑定的根；Electron main 通过四个精确 IPC 提供有界惰性列表与 UTF-8/SHA-256 只读打开。renderer 无 Node、绝对路径或 handle；保存、Agent 文件工具、Terminal、Git、搜索、重命名/删除继续关闭 |
