...
| P7.5 Linux Desktop Packaging R0 | AppDir 与离线 payload staging 合同已实现；Linux 构建与生命周期未验收 | 新增 Linux `linux-x64` Electron AppDir 打包合同、XDG 数据目录解析、Linux 路径大小写语义、进程组 RuntimeHost 原型和 `build_p7_5_linux_desktop_payload.py` 闭集 manifest/SHA staging。尚无 Ubuntu 构建产物、AppImage/deb、签名或真实 Linux VM 证据；详见 `docs/architecture/p7-5-linux-desktop-packaging.md` |
