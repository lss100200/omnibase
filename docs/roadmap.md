...
> **当前 P7.3 状态**：已从 `main@5ca173a` 完成统一 Workspace 热插拔平台源码实现与本地源码门禁。P7.3 单版本完整交付五类适配器、统一 registry/lifecycle、权限/预算、workload+network lease/fencing、撤销、故障隔离、reconciliation、恢复、设置中心与紧急停止；P7.4 只做加固与验收。产品法见 `docs/architecture/p7-3-workspace-hot-plug-platform.md` 与 INV-088；源码提交身份、fresh Windows 安装包和单实例隔离产品证据仍待生成，尚未签发安装产品或生产验收。
