...
> **当前 P7.2 状态**：从 `main@cb1295b` 完成 Workbench Refinement & Workspace Composition。紧凑/舒适/专注显示、中央全屏设置中心、Application/Workspace 设置分离、不可变 `standard-workbench`、版本化 Workspace derived Profile、闭集 Slot Registry 与 exact-SHA Owner review/approve/reject/rollback 已通过源码审计和单实例 Windows Sandbox 真实 Electron 工程验收。R0 首帧 Explorer 恢复缺陷已前向修复；R1 EXE 为 `55e45d2...ce568`，卸载 exit 0 且进程/监听清零。源码与 Windows 工程证据绑定 `8cb0020`，两组远端 Linux CI 全绿并通过 PR #49 合入 `main@5ca173a`；权威登记见 `docs/reviews/p7-2-workspace-composition-engineering-acceptance-r0.md`。产品法见 `docs/architecture/p7-2-workspace-composition.md` 与 INV-087；P3.4 的身份、scope、CAS/fencing、能力分离、审批绑定、append-only audit 和 no-auto-replay 语义保留。dirty/unsigned 包不等于生产发布。
> **当前 P7.3/P7.4 状态**：P7.3 统一 Workspace 热插拔平台与 P7.4 加固源码已推进至 clean `a17d03d`；clean `23d99a3` 又补齐 repository-enforced Settings 渲染/无障碍门禁，19 个视图在 1440x900、1024x700、200% 等效视口完成 9/9 自动检查与六张 Codex 目视截图。desktop schema v12 Network Lease 单调 fencing、五类 package-bound workload、兼容/规模/攻击/恢复及 500 组件/1,510 版本/20 Workspace/2,000 安装的 bounded certification 已通过；fresh unsigned EXE/MSI 已生成。单实例 Windows R0 只证明安装、真实 Electron/loopback、Owner/Workspace 初始化、Settings 与五类 Catalog 渲染，未完成五类 install/invoke、升级/回滚、紧急停止、恢复和卸载，故 `P7_4_WINDOWS_PRODUCT_ACCEPTANCE_INCOMPLETE`，不得签发 P7.4 acceptance。证据登记见 `docs/reviews/p7-4-hardening-windows-evidence-r0.md`。同机可信 helper 不等于独立 P34 provider/OS 隔离，后者继续 NOT_PROVEN。产品法为 `docs/architecture/p7-3-workspace-hot-plug-platform.md`、INV-088、`docs/architecture/p7-4-hot-plug-hardening-and-product-acceptance.md` 与 INV-089。签名、Marketplace 和生产发布均未签发。
...
| P7.2：Workbench Composition Foundation | 已通过 PR #49 合入 `main@5ca173a` | 中央设置中心、Application/Workspace 分离、密度/专注模式、不可变标准工作台、版本化 Workspace Profile、闭集 Slot、AI proposal、Owner exact-SHA approve/reject、rollback-as-new-revision 与 append-only audit 已完成真实 Electron 工程验收；登记见 `docs/reviews/p7-2-workspace-composition-engineering-acceptance-r0.md`。它是热插拔控制面基础，不是假装已经执行任意插件 |
| P7.3：Complete Workspace Hot-Plug Platform | 源码提交 `cc9c0d3` 与 fresh unsigned r6 包已生成；单实例隔离产品验收待执行 | 在一个大版本内完整交付统一 manifest/registry、安装与依赖、Slot 绑定、启停/升级/回滚/撤销、UI/Skill/MCP/Sandbox/trusted local-app 五类适配器、能力授权与预算、workload identity/lease/fencing、健康检查、故障隔离、reconciliation、设置中心管理与紧急停止。AI 只提案，Owner 审批 exact diff/digest；禁止 renderer 任意 JS、远程 iframe/CDN 和自动重放 `pending/unknown` |
| P7.4：Hot-Plug Hardening and Product Acceptance | clean 源码、bounded certification 与自动 Settings rendering gate 已通过；Windows R0 不完整，验收未签发 | `a17d03d` 已通过 schema v12 fencing、兼容/规模/攻击/恢复及 500 组件规模门槛；`23d99a3` 将 1440x900、1024x700、200% 等效视口、键盘/焦点、reduced-motion、forced-colors、axe 与裁切/重叠检查纳入 CI，9/9 通过。fresh unsigned 包的单实例 R0 只走到五类 Catalog，未执行完整安装/调用/恢复/卸载旅程。登记见 `docs/reviews/p7-4-hardening-windows-evidence-r0.md`。Sandbox package-bound zero-import WASM 与同机 helper 不得冒充独立 P34 provider 隔离；不得首次实现 registry、lifecycle、adapter、permission、revocation 或 recovery 核心路径 |
| P7.5 Linux Desktop Packaging R1 | Ubuntu AppDir 构建自动化已就绪；Linux 构建产物与生命周期未验收 | 在 R0 合同上新增 POSIX Python 3.12 backend builder、离线 staged desktop build、固定 Electron `v43.4.0` Linux x64 SHA 与 Ubuntu 22.04/24.04 `workflow_dispatch` artifact 产出；AppDir 强制消费同仓库 Actions run 中显式选择、绑定 bundle/tree SHA-256 且验证通过的 P7.3 十包组件 artifact。尚无已审核的 Ubuntu AppDir、AppImage/deb、签名或真实 Linux VM 证据；详见 `docs/architecture/p7-5-linux-desktop-packaging.md` |
