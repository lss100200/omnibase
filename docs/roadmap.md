...
| P7.4：Hot-Plug Hardening and Product Acceptance | clean 源码与 bounded certification 已通过；Windows R0 不完整，验收未签发 | `a17d03d` 已通过 schema v12 fencing、兼容/规模/攻击/恢复、Settings 可访问性及 500 组件规模门槛；fresh unsigned 包的单实例 R0 只走到五类 Catalog，未执行完整安装/调用/恢复/卸载旅程。登记见 `docs/reviews/p7-4-hardening-windows-evidence-r0.md`。Sandbox package-bound zero-import WASM 与同机 helper 不得冒充独立 P34 provider 隔离；不得首次实现 registry、lifecycle、adapter、permission、revocation 或 recovery 核心路径 |
