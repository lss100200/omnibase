...
> **当前 P7.3/P7.4 状态**：P7.3 统一 Workspace 热插拔平台源码已提交为 `cc9c0d3`，fresh unsigned r6 EXE/MSI 已生成；单实例 fresh Windows 产品验收尚未执行。P7.4 已授权进入加固，当前以 desktop schema v12 修复 Network Lease 单调 fencing，并建立兼容、规模、soak、攻击、恢复、Settings 可访问性与 Windows receipt 门槛。Sandbox 源码路径已绑定并执行 package 内版本不同的 zero-import `payload/workload.wasm`，但同机可信 helper 不等于独立 P34 provider/OS 隔离，后者继续 NOT_PROVEN。P7.3 产品法为 `docs/architecture/p7-3-workspace-hot-plug-platform.md` 与 INV-088；P7.4 产品法为 `docs/architecture/p7-4-hot-plug-hardening-and-product-acceptance.md` 与 INV-089。安装产品、签名、Marketplace 和生产发布均未签发。
...
| P7.4：Hot-Plug Hardening and Product Acceptance | 已授权并进入源码加固；验收未签发 | 只负责兼容矩阵、性能与规模、崩溃/攻击/恢复演练、迁移、签名/分发/Marketplace 外部证据和 UX 精修；Sandbox package-bound zero-import WASM 与同机 helper 可形成源码/工程证据，但不能冒充独立 P34 provider 隔离。产品法为 `docs/architecture/p7-4-hot-plug-hardening-and-product-acceptance.md` 与 INV-089。不得首次实现 registry、lifecycle、adapter、permission、revocation 或 recovery 核心路径 |
