...
export default function DashboardPage() {
  ...
  return (
    <div className="production-workbench fade-up flex h-[calc(100vh-6rem)] min-h-[36rem] flex-col gap-3">
      <section
        className="flex shrink-0 flex-wrap items-center gap-2 rounded-xl border border-border/75 bg-card/75 px-2.5 py-2 shadow-[0_18px_50px_-42px_rgba(0,0,0,.55)]"
        aria-label="当前工作上下文"
      >
        <WorkspaceSelector
      ...
      <div className="grid min-h-0 flex-1 gap-3 overflow-hidden xl:grid-cols-[minmax(0,1fr)_21rem]">
        ...
        <aside className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-border/80 bg-card/75 shadow-[0_24px_70px_-55px_rgba(0,0,0,.5)]">
          <header className="flex h-14 shrink-0 items-center justify-between border-b border-border/75 px-4">
            ...
            <span className="bg-primary/8 flex h-8 w-8 items-center justify-center rounded-lg text-primary">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-3">
            <RailSection title="当前 Workspace" actionHref="/spaces" actionLabel="管理">
              {currentWorkspace ? (
                ...
                >
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    <span className="mt-1 block truncate font-mono text-[8px] uppercase text-muted-foreground">
                      {currentWorkspace.desired_state} /{' '}
                      {currentWorkspace.observed_state || 'unknown'}
                    </span>
                  </span>
                ...
                <EmptyRailState
                  ...
                  description={
                    workspacesError ? '数据状态未知' : '创建一个受控工作空间开始组织项目'
                  }
                  href="/spaces"
                  action="打开 Workspace"
                />
            ...
            <RailSection title="数据工作台" actionHref="/database" actionLabel="打开数据库">
              <Link
                href="/database"
                className="group flex items-center gap-3 rounded-xl border border-border bg-muted/25 p-3 transition-colors hover:border-foreground/25 hover:bg-muted"
              >
  ...
  return (
    <div className="rounded-xl border border-border/65 bg-background/30 p-2.5">
      <div className="flex items-center justify-between gap-2">
        <Icon
          className={cn('h-3.5 w-3.5', tone === 'primary' ? 'text-primary' : 'text-foreground')}
        />
        <span
          className={cn(
...
function EmptyRailState({
  ...
  return (
    <div className="rounded-xl border border-dashed border-border/65 bg-muted/10 p-3 text-center">
      ...
      <Link
        href={href}
        className="mt-2 inline-flex items-center gap-1 text-[8px] text-primary hover:underline"
      >
        {action}
...
function PreviewBadge({ label, className }: { label: string; className?: string }) {
  return (
    <span
      className={cn(
